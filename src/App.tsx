...
function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('@demands:data');
    if (saved) {
      setDemands(JSON.parse(saved));
    }
  }, []);

  // Save to localStorage when demands change
  useEffect(() => {
    localStorage.setItem('@demands:data', JSON.stringify(demands));
  }, [demands]);
  ...
  const filteredDemands = demands.filter(d => filter === 'Todas' || d.status === filter);

  // Stats
  const totalDemands = demands.length;
  const pendingDemands = demands.filter(d => d.status === 'Pendente').length;
  const inProgressDemands = demands.filter(d => d.status === 'Em andamento').length;
  ...
  return (
    <div className="app-container">
      <header className="header">
        <div>
          <h1>WCC Manutenção</h1>
          <p>Sistema Centralizado de Demandas da Indústria</p>
        </div>
      ...
      <div className="glass-panel" style={{ padding: '2rem' }}>
        ...
        {filteredDemands.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem 0', color: 'var(--text-secondary)' }}>
            ...
            <p>Tudo está funcionando perfeitamente por aqui.</p>
          </div>
        ) : (
